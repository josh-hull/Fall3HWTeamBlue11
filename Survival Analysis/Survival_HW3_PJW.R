...
back.model <- step(full.model, direction = "backward", k=qchisq(0.03,1,lower.tail = FALSE))
summary(back.model)
#coxph(formula = Surv(hour, motor_fail) ~ age + servo + slope, data = data)
...
# Check assumption 
# linearity - age, slope
pump.ph <- coxph(Surv(start,stop,motor_fail) ~ age + servo + trashrack + slope+indicator, data = dat)
