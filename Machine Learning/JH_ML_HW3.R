...
# Read in the data
ins_t = read.csv("https://raw.githubusercontent.com/josh-hull/Fall3HWTeamBlue11/main/Machine%20Learning/insurance_t.csv")

ins_v = read.csv("https://raw.githubusercontent.com/josh-hull/Fall3HWTeamBlue11/main/Machine%20Learning/insurance_v.csv")


#####################################################################
###           Dr. LaBarr's method of imputation                   ###
#####################################################################
...
########################################################################
###        Neural Net on all variables... w/ standardization         ###
########################################################################
...
train_x <- model.matrix(INS ~ SAVBAL + DDABAL + CDBAL + DDA + MMBAL + ACCTAGE + MM + CHECKS + CCBAL + DEPAMT + ATMAMT, data = ins_t)[, -1]
#train_x <- model.matrix(INS ~ ., data = ins_t)[, -1]
train_y <- (ins_t$INS)

xgb_ins <- xgboost(data = train_x, label = train_y, subsample = .75, nrounds = 16, eta = 0.2, max_depth = 5, eval.metric = "auc", objective = "binary:logistic")

set.seed(12345)
##P-values
phat_xgb <- predict(xgb_ins, newdata = train_x, type = 'prob')

##Final ROC Curve
#Final Model AUC: 
pred <- prediction(list(phat_xgb), ins_t$INS)
perf <- performance(pred, measure = "tpr", x.measure = "fpr")
plot(perf, lwd = 3, colorize = TRUE, colorkey = TRUE, colorize.palette = rev(gray.colors(256)))
abline(a = 0, b = 1, lty = 3)
AUROC(ins_t$INS, phat_xgb)


### running final model on validation ###
valid_x <- model.matrix(INS ~ SAVBAL + DDABAL + CDBAL + DDA + MMBAL + ACCTAGE + MM + CHECKS + CCBAL + DEPAMT + ATMAMT, data = ins_v)[, -1]
#valid_x <- model.matrix(INS ~ ., data = ins_v)[, -1]
valid_y <- (ins_v$INS)

set.seed(12345)

##P-values
phat_xgb <- predict(xgb_ins, newdata = valid_x, type = 'prob')

##Final ROC Curve
#Final Model AUC: 
pred <- prediction(list(phat_xgb), valid_y)
perf <- performance(pred, measure = "tpr", x.measure = "fpr")
plot(perf, lwd = 3, colorize = TRUE, colorkey = TRUE, colorize.palette = rev(gray.colors(256)))
abline(a = 0, b = 1, lty = 3)
